...
{
    public class RoleHandler : SiGLHandlerBase
    {
        #region GetMethods
        ...
        [HttpOperation(HttpMethod.GET, ForUriName = "GetDMRole")]
        public OperationResult GetDMRole(Int32 dataManagerId)
        {
            role anEntity = null;
            ...
            try
            {
                ...
                using (SiGLAgent sa = new SiGLAgent())
                {
                    anEntity = sa.Select<data_manager>().Include(i => i.role).FirstOrDefault(i => i.data_manager_id == dataManagerId).role;
                    if (anEntity == null) throw new WiM.Exceptions.NotFoundRequestException();
                    sm(sa.Messages);
        ...
        public OperationResult POST(role anEntity)
        {
            try
            {
                if (string.IsNullOrEmpty(anEntity.role_name))
